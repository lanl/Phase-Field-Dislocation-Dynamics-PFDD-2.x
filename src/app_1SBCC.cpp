...
App1SBCC::App1SBCC(PFDD_C *pfdd_p, int narg, char **arg) :
  App(pfdd_p,narg,arg)
{
  ndouble = 0;
  timestep = 1.0;
  ...
  if(setup_flag == -1)
    setup_flag = 3; //default to screw dipole
  if(core_flag == -1)
...
/* ----------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
void App1SBCC::input_app(char *command, int narg, char **arg)
{
  error->all(FLERR,"Unrecognized command");
}

void App1SBCC::set_slip()
{
  int i, j, k;
  double s3;
...
double App1SBCC::compute_mean_xi(int n, int flag)
{
  int lN1 = fft->local_x;
...
double App1SBCC::compute_stress(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
double App1SBCC::compute_theta(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
double App1SBCC::compute_usfe(int n, int flag)
{
  int N1 = fft->nx;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
double App1SBCC::compute_strain(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
void App1SBCC::stats(char *strtmp)
{
  char *strpnt = strtmp;
...
void App1SBCC::stats_header(char *strtmp)
{
  char *strpnt = strtmp;
