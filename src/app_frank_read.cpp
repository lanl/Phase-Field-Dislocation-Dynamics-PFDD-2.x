...
AppFrankRead::AppFrankRead(PFDD_C *pfdd_p, int narg, char **arg) :
  App(pfdd_p,narg,arg)
{
  ndouble = 0;
  timestep = 1.0;
...
/* ----------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
void AppFrankRead::input_app(char *command, int narg, char **arg)
{
  error->all(FLERR,"Unrecognized command");
}

void AppFrankRead::set_slip()
{
  int i, j, k;
  double s3;
...
/* ----------------------------------------------------------------------
   It creates a file with the initial
   values of the order parameters
   ------------------------------------------------------------------------- */

void AppFrankRead::initial_sxtal()
{
...
double AppFrankRead::compute_mean_xi(int n, int flag)
{
  int lN1 = fft->local_x;
  ...
  else if(flag == 4){ // 1 slip system, imaginary value
    slip = 0;
...
double AppFrankRead::compute_stress(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
double AppFrankRead::compute_theta(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
double AppFrankRead::compute_strain(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
void AppFrankRead::stats(char *strtmp)
{
  char *strpnt = strtmp;
...
void AppFrankRead::stats_header(char *strtmp)
{
  char *strpnt = strtmp;
