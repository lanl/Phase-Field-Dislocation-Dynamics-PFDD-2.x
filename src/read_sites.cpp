...
ReadSites::ReadSites(PFDD_C *pfdd_p) : Pointers(pfdd_p)
{
...
void ReadSites::command(int narg, char **arg)
{
  ...
  if (fft == NULL) error->all(FLERR,"Read_sites command before fft_style set");
  ...
  if (neighflag) {
    CreateSites *cs = new CreateSites(pfdd_p);
    cs->ghosts_from_connectivity(app,app->neighshell);
...
void ReadSites::header()
{
  ...
  while (1) {
    ...
    if (strstr(line,"dimension")) {
      ...
      if (fft->box_exist && dimension != fft->dimension)
	error->all(FLERR,"Data file dimension does not match existing box");
      fft->dimension = dimension;
	...
	error->all(FLERR,"Off-lattice application data file "
      ...
      if (app->sites_exist && maxneigh != app->maxneigh)
	error->all(FLERR,"Data file maxneigh setting does not match existing sites");
    } else if (strstr(line,"xlo xhi")) {
      sscanf(line,"%lg %lg",&boxxlo,&boxxhi);
      if (fft->box_exist && (fabs(fft->boxxlo-boxxlo) > EPSILON ||
				fabs(fft->boxxhi-boxxhi) > EPSILON))
	  error->all(FLERR,"Data file simluation box different that current box");
    } else if (strstr(line,"ylo yhi")) {
      sscanf(line,"%lg %lg",&boxylo,&boxyhi);
      if (fft->box_exist && (fabs(fft->boxylo-boxylo) > EPSILON ||
				fabs(fft->boxyhi-boxyhi) > EPSILON))
	  error->all(FLERR,"Data file simluation box different that current box");
    } else if (strstr(line,"zlo zhi")) {
      sscanf(line,"%lg %lg",&boxzlo,&boxzhi);
      if (fft->box_exist && (fabs(fft->boxzlo-boxzlo) > EPSILON ||
				fabs(fft->boxzhi-boxzhi) > EPSILON))
	  error->all(FLERR,"Data file simluation box different that current box");
    } else break;
  }
  ...
  while (nread < nglobal) {
    ...
    *next = '\n';
    ...
    for (int i = 0; i < nchunk; i++) {
	  ...
	  y >= subylo && y < subyhi &&
	  z >= subzlo && z < subzhi) {
	if (latticeflag) app->add_site(id,x,y,z);
	else app->add_site(id,x,y,z);
      }
...
void ReadSites::neighbors()
{
  ...
  while (nread < nglobal) {
	...
	app->add_neighbors(loc->second,nvalues,values);
      }
  ...
  for (i = 0; i < app->nlocal; i++)
    ncount += app->numneigh[i];
  bigint ntotal;
  MPI_Allreduce(&ncount,&ntotal,1,MPI_PFDD_BIGINT,MPI_SUM,world);
...
void ReadSites::values()
{
  ...
  while (nread < nglobal) {
    ...
    *next = '\n';
	...
	if (latticeflag) app->add_values(loc->second,values);
	else app->add_values(loc->second,values);
      }
