...
AppBCCEdgePlanar::AppBCCEdgePlanar(PFDD_C *pfdd_p, int narg, char **arg) :
  App(pfdd_p,narg,arg)
{
  ndouble = 0;
  timestep = 1.0;
  ...
  if(setup_flag == -1)
    setup_flag = 2; //default to edge dipole
  if(core_flag == -1)
...
AppBCCEdgePlanar::~AppBCCEdgePlanar()
{
  delete [] sites;
}
...
void AppBCCEdgePlanar::grow_app()
{
  darray = fft->xi;
}
...
void AppBCCEdgePlanar::init_app()
{
  delete [] sites;
...
/* ----------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
void AppBCCEdgePlanar::input_app(char *command, int narg, char **arg)
{
  error->all(FLERR,"Unrecognized command");
}

void AppBCCEdgePlanar::set_slip()
{
  int i, j, k;
  double s3;
...
double AppBCCEdgePlanar::compute_mean_xi(int n, int flag)
{
  int lN1 = fft->local_x;
...
double AppBCCEdgePlanar::compute_stress(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
double AppBCCEdgePlanar::compute_theta(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
double AppBCCEdgePlanar::compute_usfe(int n, int flag)
{
  int N1 = fft->nx;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
double AppBCCEdgePlanar::compute_strain(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
void AppBCCEdgePlanar::stats(char *strtmp)
{
  char *strpnt = strtmp;
...
void AppBCCEdgePlanar::stats_header(char *strtmp)
{
  char *strpnt = strtmp;
