...
Finish::Finish (PFDD_C *pfdd_p, int flag) : Pointers(pfdd_p)
{
  double time,tmp;
  ...
  double time_other = timer->array[TIME_LOOP] -
    (timer->array[TIME_SOLVE] + timer->array[TIME_COMM] + 
     timer->array[TIME_FFT] + timer->array[TIME_OUTPUT] + 
  ...
  time = timer->array[TIME_FFT];
  MPI_Allreduce(&time,&tmp,1,MPI_DOUBLE,MPI_SUM,world);
  time = tmp/nprocs;
  if (me == 0) {
    if (screen) 
      fprintf(screen,"FFT time (%%) = %g (%g)\n",time,time/time_loop*100.0);
    if (logfile) 
      fprintf(logfile,"FFT time (%%) = %g (%g)\n",time,time/time_loop*100.0);
  }
