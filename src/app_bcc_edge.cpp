...
AppBCCEdge::AppBCCEdge(PFDD_C *pfdd_p, int narg, char **arg) :
  App(pfdd_p,narg,arg)
{
  ndouble = 0;
  timestep = 1.0;
...
/* ----------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
void AppBCCEdge::input_app(char *command, int narg, char **arg)
{
  error->all(FLERR,"Unrecognized command");
}

void AppBCCEdge::set_slip()
{
  int i, j, k;
  double s3;
...
double AppBCCEdge::compute_mean_xi(int n, int flag)
{
  int lN1 = fft->local_x;
...
double AppBCCEdge::compute_stress(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
double AppBCCEdge::compute_theta(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
double AppBCCEdge::compute_usfe(int n, int flag)
{
  int N1 = fft->nx;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
double AppBCCEdge::compute_strain(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
void AppBCCEdge::stats(char *strtmp)
{
  char *strpnt = strtmp;
...
void AppBCCEdge::stats_header(char *strtmp)
{
  char *strpnt = strtmp;
